  ...
  navbarMenu(
    ...
    tabPanel(
      ...
      mainPanel(
        ...
        plotOutput(outputId = "mapedit_finished")
      )
    )
  ),
...
server <- function(input, output) {
  ...
  output$myshapefile <- renderPlot({
    ...
    if (input$composition == "CIR") {
      plot(my_raster(), rgb = c(1, 2, 3), main = "CIR") +
        plot(ndvi,
             main = "NDVI",
             col = hcl.colors(10, palette = "RdYlGn"))
    }
  ...
  output$mapedit_finished <- renderPlot({
    calc_ndvi_map = function(img_map) {
      (img_map[1] - img_map[2]) / (img_map[1] + img_map[2])
    }
    ndvi_map = st_apply(my_raster_map(),
                        MARGIN = c("x", "y"),
                        FUN = calc_ndvi_map)
